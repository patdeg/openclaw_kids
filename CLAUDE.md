...
- **Known-bad OpenClaw versions**:
  ...
  - `2026.5.x` (latest: `2026.5.7` as of 2026-05-10) — **not yet tested on kids' hardware**. Notable changes: `openai-codex/*` model IDs auto-normalize to `openai/*` (non-breaking, but prefer `openai/gpt-5.4` in new configs going forward); Node.js minimum raised to 22.16+ (`node:22-bookworm` base image meets this); no changes to pi-ai/openai-completions path found — pi-ai patch still required. Use the bump procedure in `Dockerfile.openclaw` before rolling out.
