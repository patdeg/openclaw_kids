...
- AI assistant name (you'll choose this later during configuration)
````
...
to pin a specific version temporarily, you can pass
`--build-arg OPENCLAW_VERSION=2026.2.26` to the Docker build — but
that's a troubleshooting step, not something you need to do now.
...
The `bootstrap.sh` script handles all of this automatically —
...
> **Don't run anything yet!** This section is just explanation — there
> are no commands to run here. Read it so you understand what
> `configure.sh` and `bootstrap.sh` will set up in the next stage.

The main config template lives at `config/openclaw.kids.json.example`.
Here's what each section does:

### IDENTITY — Who Your Assistant Is

```json
// EXAMPLE — don't edit this directly, configure.sh handles it
"identity": {
  "name": "YOUR_NAME_HERE",
  "description": "Personal AI assistant for school, gaming, and projects"
}
```

The name and one-line description that define your assistant's persona.
You'll choose your own name when you run `configure.sh` in the next stage.
...
The template lives at `config/FAMILY_COMPASS.md.example` — read it
when you're curious, it's the most important file in this project.
...
```json
// EXAMPLE — these are configured in your local openclaw.kids.json
"skills": {
  "entries": {
    "school": { "enabled": true },
...
If you're curious, peek inside the `skills/` directory in the project —
each subfolder is a skill with its own scripts and config.

### HEARTBEAT — The Wake-Up Loop

```json
// EXAMPLE — configure.sh sets your active hours
"heartbeat": {
  "enabled": true,
  "interval": "1h",
...
```json
// EXAMPLE — channels are pre-configured, you just add tokens in .env
"channels": [
...
- `~/AGENTS.md` (created earlier) — your machine and personal details
...
**Everything from here is done by running scripts — you don't need to
edit any of the files shown above by hand.**

---

## Stage 11: Configure and Deploy Your AI Assistant

This is where you make it yours. Two scripts do all the work:
1. **`configure.sh`** — asks you a few questions and generates your config files
2. **`bootstrap.sh`** — builds the containers and starts everything

That's it. Run those two and you have a working AI assistant.
